...
class FileManager:
    ...
    def _get_sort_lista_plikow(lista_plikow, names_for_size_in_model_map):
        def _get_size(plik, names_for_size_in_model_map):
        ...
        return sorted(lista_plikow, key=lambda x : _get_size(x, names_for_size_in_model_map), reverse=True)

    @staticmethod
    def get_files_list(folder_path, ignore_file_names, ignorowane_rozszerzenia, ignorowane_foldery, names_for_size_in_model_map):
        lista_plikow = FileManager._get_all_files_paths_from_directory(folder_path, ignorowane_rozszerzenia=ignorowane_rozszerzenia,
                                                                       ignore_file_names=ignore_file_names, ignorowane_foldery=ignorowane_foldery)
        return FileManager._get_sort_lista_plikow(lista_plikow, names_for_size_in_model_map)
