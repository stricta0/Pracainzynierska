from definicje_wykresow import DefinicjeWykresow
from creator_wykresow import CreatorWykresow
from file_menadger import FileMenadger
from stworz_plik_statystyk_z_folderu import CreateStatisticsFile
import json
import os
...
class PanelKoncowyDoubleDescent:
    ...
    def stworz_wykresy_z_danych_folderu_double_descent_best_models(self, save_path=None, lista_plikow=None, yscale="linear", xscale="linear"):
        ...
        definicje_wykresow = DefinicjeWykresow(lista_plikow)
    ...
    def stworz_wykresy_z_danych_folderu_double_descent_po_liczbie_epoch(self, save_path=None, lista_plikow=None, yscale="linear",
                                                       xscale="linear"):
        if lista_plikow is None:
            lista_plikow = self.lista_plikow
        if save_path is None:
...
def przeprowadz_analize_double_descent_w_folderze(folder_path, save_wyniki_path):
    ...
    panel.stworz_wykresy_z_danych_folderu_double_descent_po_liczbie_epoch(yscale="linear", xscale="linear")
    panel.stworz_wykresy_z_danych_folderu_double_descent_po_liczbie_epoch(yscale="linear", xscale="log")
    panel.stworz_wykresy_z_danych_folderu_double_descent_po_liczbie_epoch(yscale="log", xscale="linear")
    panel.stworz_wykresy_z_danych_folderu_double_descent_po_liczbie_epoch(yscale="log", xscale="log")
    panel.zapisz_statystyki_z_danych_folderu_double_descent()

from pathlib import Path

def przeprowadz_analize_double_descent_w_folderze_rekurencyjnie(folder_path, log_warn=False):
    """
    ...
    if has_any_file:
        try:
            ...
            przeprowadz_analize_double_descent_w_folderze(str(root), str(root))
    ...
    for sub in subdirs:
        ...
        if sub.is_symlink():
            continue
        przeprowadz_analize_double_descent_w_folderze_rekurencyjnie(sub)



if __name__ == "__main__":
    folder_path = "/home/miku/PycharmProjects/Pracainzynierska/wyniki_eksperymentow/wykresy/"
    przeprowadz_analize_double_descent_w_folderze_rekurencyjnie(folder_path, True)
