...
class RRF(nn.Module):
    ...
    Random ReLU Features:
      phi(x) = ReLU(x W + b), gdzie W, b są losowe i zamrożone (bez uczenia).
      Uczona jest wyłącznie warstwa wyjściowa (liniowa) mapująca phi(x) -> logits.
    """
    def __init__(self, input_dim: int = 28*28, hidden: int = 128, num_classes: int = 10):
        super().__init__()
        # Losowa projekcja + ReLU; parametry zamrożone.
        self.fc_random = nn.Linear(input_dim, hidden)
        # He init dla ReLU
        with torch.no_grad():
            nn.init.normal_(self.fc_random.weight, mean=0.0, std=(2.0 / input_dim) ** 0.5) #po sumie wszystkich wag, kwadratow z std dostajemy około 2 z czego połowa jest zerowana przez relu co daje plus minus 1
            nn.init.zeros_(self.fc_random.bias)
        for p in self.fc_random.parameters():
            p.requires_grad = False  # zamrożenie RRF

        # Jedyna uczona warstwa
        self.fc_out = nn.Linear(hidden, num_classes)

    def forward(self, x):
        # x: (B, 1, 28, 28) -> (B, 784)
        x = x.flatten(1)
        h = F.relu(self.fc_random(x))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Random ReLU Features (RRF) on MNIST/Fashion/EMNIST")
    parser.add_argument("--dataset", type=str, default="mnist",
                        choices=["mnist", "fashion", "emnist_balanced", "emnist_byclass"])
    parser.add_argument("--hidden", type=int, default=100, help="liczba losowych cech (neurons in random hidden layer)")
    parser.add_argument("--batch_size", type=int, default=128)
    parser.add_argument("--epochs", type=int, default=10000)
    parser.add_argument("--patience", type=int, default=1000)
    parser.add_argument("--lr", type=float, default=0.001)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--momentum", type=int, default=0.95)  # zachowano identycznie jak w oryginale
